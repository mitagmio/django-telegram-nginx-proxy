        ...
        if len(Transactions) > 0:
            ...
            for t in Transactions:
                ...
                if pay_value > 0 :
                    bal_before = u.balance
                    u.balance += pay_value
                    text = '💵 Ваш платеж на сумму <code>{}</code> USDT зачислен.\n\nТекущий баланс составляет <code>{}</code> USDT\n\n‼️ <b>ТЕПЕРЬ ВЫ МОЖЕТЕ ПРИОБРЕСТИ ИНТЕРЕСУЮЩИЙ ВАС ПРОДУКТ ВЫБРАВ НУЖНЫЙ ПУНКТ МЕНЮ.</b>'.format(
                        pay_value, u.balance)
                    _send_message(
                        user_id=u.user_id,
                        text=text,
...
def send_selected_BlackFriday_manual() -> None:
    ...
    try:
        ...
        Users = User.objects.filter(marker__in=['selected', 'metamask, selected']).filter(execute_selected_time=0)#.exclude(user_id__in=admin_ids)#.filter(username__in=['zhukov_zhukov','mitagmio'])#
    ...
    if len(Users) > 0:
        ...
        for u in Users:
            ...
            broadcast_message(
                user_ids=[u.user_id],
                text='''Спасибо, что вы были членом коммьюнити SELECTED. Мы хотим, чтобы вы к нам вернулись. Предлагаем вам воспользоваться нашим предложением:

90 дней в SELECTED за 300$

Это эквивалентно 50% скидке от обычной цены. Эта возможность активна до конца сегодняшнего дня. Не упустите шанс провести все лето в SELECTED, жмите на кнопку ниже.''',
                entities = None,
...
def send_invoice_7_selected() -> None:
    ...
    if len(Users) > 0:
        bts = [
            ...
            [
                {'text': '🔖 Напоминать позже', 'callback_data': 'Старт'},
                {'text': '💢 Не напоминать', 'callback_data': 'Не_напоминать'}
            ],
            [
                {'text':'⏮ В начало', 'callback_data':'Старт'},
...
def send_invoice_3_selected() -> None:
    ...
    if len(Users) > 0:
        bts = [
            ...
            [
                {'text': '💸 Продлить доступ на 90д', 'callback_data': 'Купить_Селектед_120'},
            ],
            [
                {'text':'⏮ В начало', 'callback_data':'Старт'},
                {'text':'⏪ Назад', 'callback_data':'Меню'}
...
Через три дня заканчивается ваша подписка SELECTED.
Надеемся, что вам у нас понравилось, и вы хотите ее продлить на следующий период.
...
Через один дня заканчивается ваша подписка SELECTED.
Надеемся, что вам у нас понравилось, и вы хотите ее продлить на следующий период.
